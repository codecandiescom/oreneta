...
public class SnapshotFlowTableModel extends OrenetaTableModel {
	Snapshot snapshot;
	SimpleDateFormat sdf;
	
	////////////////////////////////////////////////////////////////////////////
	public SnapshotFlowTableModel (Snapshot snapshot) {
		...
		columnNames[12] = "Total Bytes";
		columnNames[13] = "Total Packets";
		...
		selectedColumns[8] = true;
		selectedColumns[9] = true;
  	...
  	public int getRowCount() {
  		return snapshot.getNumSnapshotFlows();
  	}
  	...
  	public Object getValueAt(int row, int col) {
  		try {
  			Flow f = snapshot.getSnapshotFlowAt (row);
   			if (f != null) {
	  			switch (col) {
	  				case 0:	// Src IP
	  					return f.src.toString().substring(1);
	  					
	  				case 1: // Src Host
	  					return f.getSrc();
	
	  				case 2: // Src Port 
	  					return new Integer(f.src_port);
	  				
	  				case 3:	// Src Service (translated port)
	  					return f.getSrcService();
	  					
	  				case 4: // Dst IP
	  					return f.dst.toString().substring(1);
	  					
	  				case 5: // Dst Host
	  					return f.getDst();
	  					
	  				case 6: // Dst Port
	  					return new Integer(f.dst_port);
	  					
	  				case 7: // Dst Service
	  					return f.getDstService();
	  					
	  				case 8: // Last Time
	  					return new String (sdf.format(new Date ((long)f.lastTime*1000), new StringBuffer(), new FieldPosition(0)).toString());
	  		
	  				case 9:	// Creation Time
	  					return new String (sdf.format(new Date ((long)f.time*1000), new StringBuffer(), new FieldPosition(0)).toString());
	
	  				case 10: // Monitor
	  					return new Boolean (f.monitor);
	  				
	  				case 11: // Color
	  					return f.color;
	  				
	  				case 12: // Total Bytes
	  					return new Long(f.total_bytes);
	  					
	  				case 13: // Total Packets
	  					return new Long(f.total_packets);
	  			}
	  		} else {
	  			columnDefault(col);
	  		}
  		...
  		if (f != null) {
  			switch (col) {
  				case 10:
		  			...
		  			f.monitor = b.booleanValue();
					if (f.monitor)
						snapshot.drawSnapshotFlow(f);
					else
						snapshot.undrawSnapshotFlow(f);

...
public class ChartNegMaxMin extends ChartNeg {
	
  ////////////////////////////////////////////////////////////////////////////
  public ChartNegMaxMin (GUI gui, String title, int param, String units) {
		...
		*/
		...
		while (li.hasNext()) {
			...
			for (int j=1; j<horzDiv; j++) {
				...
				aux = cm.getValue(j,param,1);
				if (aux == Double.MIN_VALUE) y_max = y2;
				else y_max = height/2 + um - (int)((aux / max_value) * ((double)(height - dm - um) / 2));
				
				aux = cm.getValue(j,param,2);
				if (aux == Double.MAX_VALUE) y_min = y2;
				else y_min = height/2 + um - (int)((aux / max_value) * ((double)(height - dm - um) / 2));
